...
const lcdPin_t *_rs = NULL;
const lcdPin_t *_enable = NULL;
/*const lcdPin_t *_d0 = NULL;
const lcdPin_t *_d1 = NULL;
const lcdPin_t *_d2 = NULL;
const lcdPin_t *_d3 = NULL;*/
...
void lcdSetPinDirection(const lcdPin_t *pin, t_pinDir dir);
void lcdSetPin(const lcdPin_t *pin, uint32 value);
...
void lcdPrintSignalLine(const char *name, const lcdPin_t *p);

/**
 * API:
 */
void lcdInit(const lcdPin_t *rs, const lcdPin_t *enable,
		const lcdPin_t *d0, const lcdPin_t *d1,
		const lcdPin_t *d2, const lcdPin_t *d3) {
	...
	lcdSetPinDirection(_rs, pinWrite);
//	lcdSetPinDirection(_rw, pinWrite); // optional, will only enable if needed
	lcdSetPinDirection(_enable, pinWrite);
	// in LiquidCristal,
...
void lcdBegin(uint32 cols, uint32 lines, uint32 dotsize) {
	...
	while(1) {
		lcdSetPin(_rs, trig);
		lcdSetPin(_enable, trig);
		lcdSetPin(_data_pins[0], trig);
		lcdSetPin(_data_pins[1], trig);
		lcdSetPin(_data_pins[2], trig);
		lcdSetPin(_data_pins[3], trig);
//		lcdDelayMicroSeconds(50000);
		trig = trig ? 0 : 1;
...
void write4bits(uint32 value) {
	int i;
	for (i = 0; i < 4; i++) {
		lcdSetPinDirection(_data_pins[i],pinWrite);
		lcdSetPin(_data_pins[i], (value >> i) & 0x01);
	}
...
void lcdTestBed() {
    lcdPin_t test1 = {gioPORTA,0};
	lcdPin_t test2 = {gioPORTA,2};
    lcdSetPinDirection(&test1, pinWrite);
    lcdSetPinDirection(&test2, pinWrite);
    while(1) {
